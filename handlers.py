...
async def game_handler(message: types.Message):
    await message.answer(text=f"привет,{hbold(message.from_user.full_name)}, сейчас мы поиграм в игру, которая поможет развить твой словарный запас в английском")
    dictionary = ['water drop','become','awesome','in general','siblings','fabric','chainsaw','frienship','clothes']
    dictionary_2 = ['капля',"становиться","превосходно","в общем","братья","ткань","бензопила","дружба","одежда"]
    zipka = dict(zip(dictionary, dictionary_2))
    goals = 0
    n = 0
    random_word = random.choice(list(zipka.keys()))
    while n!=10:
        await message.answer(text=random_word)
        if message.answer == random_word:
            goals+=1
            n+=1
        n+=1
